...
import catz.base.{ BoundedLeibniz => BL }

sealed abstract class BoundedLeibniz[-L <: Ξ, +H >: L <: Ξ, A >: L <: H, B >: L <: H] private[BoundedLeibniz] () { ab =>
  /**
  ...
  def substK[L1 <: Ξ, H1 >: L1 <: Ξ, F[_ >: L <: H, _ >: L1 <: H1]]: λ[`α >: L1 <: H1` => F[A, α]] ~> λ[`α >: L1 <: H1` => F[B, α]] = {
    type f[α >: L <: H] = λ[`β >: L1 <: H1` => F[A, β]] ~> λ[`β >: L1 <: H1` => F[α, β]]
    subst[f](FunctionK.id[λ[`α >: L1 <: H1` => F[A, α]]])
  }
...
//    subst[λ[α => α]](a)
  ...
  final def toUnbounded: LeibnizK[A, B] =
    ab.subst[λ[α => LeibnizK[A, α]]](LeibnizK.refl)
}
...
object BoundedLeibniz {
  ...
  private[this] val anyRefl: BL[Ξ, Ξ, Ξ, Ξ] = Refl[Ξ]()
  ...
  def unsafeForce[L <: Ξ, H >: L <: Ξ, A >: L <: H, B >: L <: H]: BoundedLeibniz[L, H, A, B] =
    anyRefl.asInstanceOf[BoundedLeibniz[L, H, A, B]]

  final def bound[L <: Ξ, H >: L <: Ξ, A >: L <: H, B >: L <: H]
  (ab: LeibnizK[A, B]): BoundedLeibniz[L, H, A, B] =
    unsafeForce[L, H, A, B]

  /**
    * Equality is reflexive relation.
    */
  def refl[A <: Ξ]: BL[A, A, A, A] =
    unsafeForce[A, A, A, A]
  ...
  /**
    ...
    */
  def refl_[L <: Ξ, H >: L <: Ξ, A >: L <: H]: BL[L, H, A, A] =
    unsafeForce[L, H, A, A]
